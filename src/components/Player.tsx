...
export function Player({ activeBlocks }: { activeBlocks: any[] }) {
        ...
        for (let cz = pz - 1; cz <= pz + 1; cz++) {
          ...
          if (chunk) {
            ...
            for (let i = 0; i < len; i++) {
              const b = chunk[i];
              if (
                Math.abs(b.position[0] - pointX) < 8 &&
  ...
  const checkSupport = (pos: number[], nearbyBlocks: any[]) => {
    ...
    return nearbyBlocks.some((b) => {
      ...
      const overlapX =
        b1_minX < b2_maxX - epsilon && b1_maxX > b2_minX + epsilon;
      const overlapZ =
        b1_minZ < b2_maxZ - epsilon && b1_maxZ > b2_minZ + epsilon;
  ...
  useFrame((state) => {
    ...
    _frontVector.y = 0;
    if (_frontVector.lengthSq() < 0.0001) {
      _frontVector.set(0, 0, -1).applyEuler(new THREE.Euler(0, camera.rotation.y, 0));
      _frontVector.y = 0;
    }
    ...
    // Rapier Raycast
    _rayOrigin.x = _start.x;
    _rayOrigin.y = _start.y;
    _rayOrigin.z = _start.z;
    _rayDir.x = _dir.x;
    _rayDir.y = _dir.y;
    _rayDir.z = _dir.z;
    ...
    const currentGhostY = ghostPosRef.current
    ...
    const evaluatePos = (testP: number[], queryPoint: THREE.Vector3) => {
      ...
      const key =
        Math.round(testP[0] * 10) +
        "_" +
        Math.round(testP[1] * 10) +
        "_" +
        Math.round(testP[2] * 10);
      ...
      if (
        checkCollision(testP, nearbyBlocks) &&
        checkSupport(testP, nearbyBlocks)
      ) {
        minScore = score;
  ...
  useFrame((state, delta) => {
    if (ghostPos && ghostRef.current) {
      ...
      ghostRef.current.rotation.x = THREE.MathUtils.lerp(
        ghostRef.current.rotation.x,
        diff.z * 0.3,
        0.1,
      );
      ghostRef.current.rotation.z = THREE.MathUtils.lerp(
        ghostRef.current.rotation.z,
        -diff.x * 0.3,
        0.1,
      );
    }
  ...
  useEffect(() => {
    if (ghostPos && !isGhostInvalid) {
      if (
        !ghostPosRef.current ||
        ghostPosRef.current[0] !== ghostPos[0] ||
        ghostPosRef.current[1] !== ghostPos[1] ||
        ghostPosRef.current[2] !== ghostPos[2]
      ) {
  ...
  useEffect(() => {
    ...
    const handleMouseDown = (e: MouseEvent) => {
      if (e.type === "mousedown" && e.isTrusted && !document.pointerLockElement)
        return;
