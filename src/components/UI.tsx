...
const CATEGORIZED_TYPES = {
  bricks: TYPES.filter(
    (t) => !t.type.startsWith("plate_") && !isFurniture(t.type),
  ),
...
export function UI() {
  ...
  const togglePerformanceMode = useStore(
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      if (key === "ArrowRight" || key === "ArrowLeft") {
        const currentColorIndex = COLORS.findIndex(
          (c) => c.hex === useStore.getState().selectedColor,
        );
    ...
    const handleWheel = (e: WheelEvent) => {
      ...
      const currentIndex = TYPES.findIndex(
        (t) => t.type === useStore.getState().selectedType,
      );
  ...
  const handleSave = useCallback(() => {
    const dataStr =
      ...
      encodeURIComponent(JSON.stringify(useStore.getState().blocks));
  ...
  return (
    <div className="absolute top-0 left-0 w-full h-full pointer-events-none flex flex-col justify-between overflow-hidden font-sans">
      ...
      <div
        className={`absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 opacity-80 mix-blend-difference pointer-events-none z-30 transition-opacity ${!hasPointerLock && !isMobile ? "hidden" : ""}`}
      >
        ...
        <div className="pointer-events-auto flex flex-col items-end gap-2">
          ...
          {!uiHidden && (
            <>
              ...
              <div
                className={`flex flex-col gap-2 items-end transition-all origin-top-right ${isMobile && !menuOpen ? "scale-0 opacity-0 absolute -z-10" : "scale-100 opacity-100 relative z-10"} ${hasPointerLock && !isMobile ? "opacity-0 scale-95 pointer-events-none" : ""}`}
              >
                      ...
                      onClick={(e) => {
                        ...
                        try {
                          ...
                          if (!document.fullscreenElement) {
                            ...
                            if (
                              navScreen.orientation &&
                              navScreen.orientation.lock
                            ) {
                              navScreen.orientation
                                .lock("landscape")
                                .catch(() => {});
                  ...
                  {isMobile && (
                    <>
                      <button
                        ...
                        className={`p-2.5 rounded-full shadow-sm border transition-all ${Math.max(history.length, 0) === 0 ? "bg-slate-100 text-slate-400 border-transparent" : "bg-white text-rose-500 border-rose-100 hover:scale-105"}`}
                      >
                      ...
                      <button
                        ...
                        className={`p-2.5 rounded-full shadow-sm border transition-all ${Math.max(redoStack.length, 0) === 0 ? "bg-slate-100 text-slate-400 border-transparent" : "bg-white text-emerald-500 border-emerald-100 hover:scale-105"}`}
                      >
                  ...
                  <button
                    ...
                    title={
                      performanceMode
                        ...
                        : "Enable Performance Mode (Low Graphics)"
                    }
                  >
                    {performanceMode ? (
                  ...
                  {!isMobile && (
                    <>
                      ...
                      <button
                        ...
                        className="p-2.5 sm:p-3 bg-white hover:bg-slate-50 text-slate-700 rounded-full shadow-md sm:shadow-lg border border-slate-100 transition-all"
                      >
      ...
      {!isMobile && !uiHidden && (
        <div
          className={`absolute left-2 md:left-0 top-[12%] sm:top-[14%] md:top-1/2 md:-translate-y-1/2 p-0 sm:p-2 md:p-6 transition-all duration-300 z-40 ${hasPointerLock && !isMobile ? "opacity-40 pointer-events-none translate-x-0" : "translate-x-0 pointer-events-auto"} max-h-[50vh] sm:max-h-[50vh] portrait:max-h-[60vh] md:h-[90vh] md:max-h-none`}
        >
            ...
            <div className="flex-1 overflow-y-auto touch-pan-y pr-1 -mr-1 space-y-1 sm:space-y-2 hide-scrollbar scroll-smooth flex flex-col items-center">
              {TYPES.map((t, index) => (
                <button
                  ...
                  title={t.label}
                >
                  <span className="absolute top-1 left-1.5 text-[0.4rem] sm:text-[0.5rem] opacity-30 font-mono z-20 pointer-events-none hidden md:block">
                    {index + 1}
                  </span>
                  {isFurniture(t.type) &&
                    !useStore.getState().furnitureUnlocked && (
      ...
      <>
        ...
        <div
          className={`absolute inset-0 z-40 pointer-events-none transition-opacity ${!isMobile || uiHidden ? "hidden" : ""}`}
        >
          {/* Left Joystick */}
          <div className="absolute left-[max(1.5rem,env(safe-area-inset-left))] pointer-events-none portrait:bottom-[max(220px,calc(220px+env(safe-area-inset-bottom)))] landscape:bottom-[max(1.5rem,calc(1.5rem+env(safe-area-inset-bottom)))]">
            <Joystick />
          </div>

          {/* Right Actions */}
          <div className="absolute right-[max(0.75rem,env(safe-area-inset-right))] pointer-events-none portrait:bottom-[max(200px,calc(200px+env(safe-area-inset-bottom)))] landscape:bottom-[max(0.75rem,calc(0.75rem+env(safe-area-inset-bottom)))]">
            <div className="pointer-events-auto flex items-end gap-1 sm:gap-2">
              ...
              <div className="flex flex-col gap-1 sm:gap-2">
                <button
                  ...
                  className={`w-10 h-10 sm:w-14 sm:h-12 rounded-xl flex items-center justify-center shadow-md touch-none transition-transform active:scale-95 border border-white/40 backdrop-blur-md ${isFlying ? "bg-pink-500 text-white" : "bg-white/95 text-slate-700"}`}
                ...
                <button
                  ...
                  onPointerDown={(e) => {
                    e.preventDefault();
                    e.stopPropagation();
                    window.dispatchEvent(new CustomEvent("mobile-delete"));
                  }}
                  className="w-10 h-10 sm:w-14 sm:h-12 bg-rose-500/90 rounded-xl flex items-center justify-center active:bg-rose-600 shadow-md text-white touch-none transition-transform active:scale-95 border border-white/40 backdrop-blur-md"
                >
              ...
              <div className="flex flex-col justify-end">
                <button
                  ...
                  onPointerDown={(e) => {
                    e.preventDefault();
                    e.stopPropagation();
                    window.dispatchEvent(new CustomEvent("mobile-place"));
                  }}
                  className="w-14 h-14 sm:w-20 sm:h-20 bg-pink-500 rounded-2xl flex items-center justify-center active:bg-pink-600 shadow-[0_4px_20px_rgba(236,72,153,0.6)] text-white touch-none transition-transform active:scale-95 border-2 border-white/30 backdrop-blur-sm mb-0.5"
                >
                  <Plus className="w-7 h-7 sm:w-10 sm:h-10" strokeWidth={3} />
                </button>
              </div>
            </div>
          </div>
        ...
        <div
          className={`absolute pointer-events-none z-50 portrait:bottom-0 landscape:bottom-2 sm:bottom-6 inset-x-0 w-full flex flex-col items-center portrait:justify-end landscape:justify-end px-1 sm:px-6 transition-all duration-300 mx-auto ${hasPointerLock && !isMobile ? "opacity-80 scale-95" : ""}`}
          style={{ paddingBottom: "env(safe-area-inset-bottom)" }}
        >
          {/* Unified Mobile Bottom Bar */}
          {isMobile && !uiHidden && (
            <div className="w-full portrait:max-w-[95vw] landscape:w-auto landscape:max-w-[240px] pointer-events-auto mx-auto flex flex-col items-center bg-white/80 backdrop-blur-md rounded-[1.25rem] landscape:rounded-xl shadow-2xl border border-white/60 p-1 landscape:p-0.5 gap-1 landscape:gap-0.5 mb-1 sm:mb-0">
              {/* Tab Switcher */}
              <div className="flex w-full bg-slate-200/40 rounded-2xl landscape:rounded-lg p-1 portrait:mb-0.5 relative mb-0.5 landscape:mb-0">
                <button
                  tabIndex={-1}
                  onFocus={(e) => e.target.blur()}
                  onPointerDown={(e) => {
                  ...
                  className={`flex-1 py-2 landscape:py-1.5 rounded-xl landscape:rounded-md text-[11px] landscape:text-xs font-black uppercase tracking-wider transition-all z-10 ${mobileTab === "bricks" ? "bg-white text-pink-500 shadow-md" : "text-slate-500"}`}
                >
                  Bricks
                </button>
                <button
                  ...
                  className={`flex-1 py-2 landscape:py-1.5 rounded-xl landscape:rounded-md text-[11px] landscape:text-xs font-black uppercase tracking-wider transition-all z-10 ${mobileTab === "colors" ? "bg-white text-pink-500 shadow-md" : "text-slate-500"}`}
                >
                  Colors
                </button>
              </div>
              ...
              {mobileTab === "bricks" && (
                <div className="w-full flex flex-col gap-1 landscape:gap-0.5">
                  {/* Category Selector */}
                  <div className="flex items-center w-full justify-between gap-1 overflow-x-auto touch-pan-x hide-scrollbar px-0.5">
                    {BRICK_CATEGORIES.map((cat) => {
                      ...
                      return (
                        <button
                          ...
                          className={`flex-1 shrink-0 flex items-center justify-center gap-1 py-1 landscape:py-0.5 rounded-xl landscape:rounded-lg transition-all ${activeCategory === cat.id ? "bg-pink-500 text-white shadow-sm" : "bg-white/40 text-slate-600"}`}
                        >
                  ...
                  <div className="w-full bg-white/40 rounded-xl landscape:rounded-lg p-1 landscape:p-0.5">
                    <div className="flex w-full overflow-x-auto touch-pan-x hide-scrollbar gap-1 snap-x snap-mandatory px-0.5">
                      {CATEGORIZED_TYPES[activeCategory].map((t) => (
                        <button
                          ...
                          key={t.type}
                          onClick={() => handleTypeSelect(t.type)}
                          className={`relative shrink-0 w-8 h-8 sm:w-11 sm:h-11 portrait:w-8 portrait:h-8 landscape:w-7 landscape:h-7 flex items-center justify-center rounded-lg landscape:rounded-md transition-all snap-center ${
                            selectedType === t.type
                              ? "bg-white shadow-md ring-2 ring-pink-500/30"
                              : "bg-transparent active:bg-black/5"
                          }`}
                        >
                          {isFurniture(t.type) && !furnitureUnlocked && (
                            <Lock className="absolute top-0.5 right-0.5 w-2 h-2 text-slate-400 opacity-60 pointer-events-none" />
                          )}
                          <div className="scale-[0.4] sm:scale-[0.6] portrait:scale-[0.4] landscape:scale-[0.35] flex items-center justify-center w-full h-full pointer-events-none">
                            <ModelIcon
                              type={t.type}
                              selected={selectedType === t.type}
                            />
              ...
              {mobileTab === "colors" && (
                <div className="w-full bg-white/40 rounded-xl landscape:rounded-lg p-1 landscape:p-0.5">
                  <div className="flex w-full overflow-x-auto touch-pan-x hide-scrollbar gap-1.5 landscape:gap-1 px-1 py-1 landscape:py-0.5 snap-x snap-mandatory">
                    {COLORS.map((c) => (
                      <button
                        tabIndex={-1}
                        onFocus={(e) => e.target.blur()}
                        ...
                        className={`relative flex-shrink-0 snap-center w-7 h-7 sm:w-10 sm:h-10 portrait:w-7 portrait:h-7 landscape:w-5 landscape:h-5 rounded-full border border-white/40 transition-transform shadow-sm ${
                          selectedColor === c.hex
                            ? "scale-110 shadow-md ring-2 ring-black/80 z-10"
                            : ""
                        }`}
          ...
          {!isMobile && (
            <div className="flex flex-row items-center justify-center w-full portrait:max-w-[90vw] landscape:w-auto landscape:max-w-[calc(100vw-360px)] sm:w-auto pointer-events-auto mx-auto mb-2 transition-all duration-300">
              <div
                className={`bg-white/80 backdrop-blur-xl p-1 sm:p-1.5 md:p-3 rounded-2xl md:rounded-3xl shadow-[0_4px_20px_rgba(0,0,0,0.1)] border border-white/50 flex w-full items-center gap-1 max-w-full overflow-hidden flex-1 md:w-auto ${uiHidden ? "hidden" : ""}`}
              >
                  ...
                  {COLORS.map((c) => (
                    <button
                      ...
                      className={`relative flex-shrink-0 snap-center w-7 h-7 sm:w-8 sm:h-8 md:w-10 md:h-10 rounded-full border border-white/40 transition-transform shadow-sm before:absolute before:content-[''] before:inset-[-8px] before:z-0 ${
                        selectedColor === c.hex
                          ? "border-white scale-110 shadow-md ring-2 ring-black/80 z-10"
                          : ""
                      }`}
                      style={{ backgroundColor: c.hex }}
                      title={c.name}
              ...
              <div className="bg-white/90 backdrop-blur-xl p-2 rounded-full shadow-xl border border-white/50 flex flex-row items-center shrink-0 ml-2 pointer-events-auto transition-all">
                <div
                  className={`flex items-center gap-1 p-1 bg-slate-100/50 rounded-full ${uiHidden ? "hidden" : ""}`}
                >
                  <button
                    tabIndex={-1}
                    onFocus={(e) => e.target.blur()}
                ...
                <div
                  className={`w-px h-8 bg-slate-300/50 mx-2 ${uiHidden ? "hidden" : ""}`}
                ></div>
                <button
                  tabIndex={-1}
                  onFocus={(e) => e.target.blur()}
      ...
      <div
        className={`absolute bottom-6 right-6 p-4 rounded-2xl bg-white/70 backdrop-blur-md border border-white/50 shadow-xl transition-opacity duration-300 z-40 hidden md:block ${isMobile ? "!hidden" : ""} ${hasPointerLock ? "opacity-20 hover:opacity-100" : "opacity-100"}`}
      >
