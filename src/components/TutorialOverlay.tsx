...
import { Gamepad2, Palette, Trash2, Plus, Sparkles, CheckCircle2, ChevronRight, Layers, RotateCcw, Eye, Rocket, Menu } from 'lucide-react';

export function TutorialOverlay() {
  const { hasSeenTutorial, setHasSeenTutorial, isMobile, blocks, selectedColor, selectedType } = useStore();
  ...
  useEffect(() => {
    ...
    return () => { checked = true; clearTimeout(timer); };
  ...
  useEffect(() => {
    ...
    const handleKey = (e: KeyboardEvent) => {
      // Step 2: Move (WASD)
      if (step === 2 && ['w','a','s','d','W','A','S','D'].includes(e.key)) {
        setStep(3);
      }
  ...
  const stepsInfo = [
    { id: 0, title: "Welcome to SkyBricks", description: "Let's learn how to build your world.", icon: Sparkles, color: "text-amber-500", bg: "bg-amber-500", border: 'border-amber-500' },
    { id: 1, title: "Look Around", description: isMobile ? "Drag on the screen to look around" : "Move your mouse to look around", icon: Eye, color: "text-sky-500", bg: "bg-sky-500", border: 'border-sky-500' },
    { id: 2, title: "Move", description: isMobile ? "Use the left joystick to move" : "Use W, A, S, D to walk", icon: Gamepad2, color: "text-blue-500", bg: "bg-blue-500", border: 'border-blue-500' },
    { id: 3, title: "Fly", description: isMobile ? "Tap the UP/DOWN arrows" : "Use Space to fly, Shift to descend", icon: Rocket, color: "text-fuchsia-500", bg: "bg-fuchsia-500", border: 'border-fuchsia-500' },
    { id: 4, title: "Place Brick", description: isMobile ? "Tap the pink + button to place" : "Click to place a brick", icon: Plus, color: "text-pink-500", bg: "bg-pink-500", border: 'border-pink-500' },
    { id: 5, title: "Demolish", description: isMobile ? "Tap the red trash button" : "Right click a brick to delete it", icon: Trash2, color: "text-red-500", bg: "bg-red-500", border: 'border-red-500' },
    { id: 6, title: "Shape", description: isMobile ? "Swipe the menu to change shape" : "Scroll wheel to cycle shapes", icon: Layers, color: "text-indigo-500", bg: "bg-indigo-500", border: 'border-indigo-500' },
    { id: 7, title: "Rotate", description: isMobile ? "Tap the rotate button to spin" : "Press 'R' to rotate", icon: RotateCcw, color: "text-teal-500", bg: "bg-teal-500", border: 'border-teal-500' },
    { id: 8, title: "Paint Brick", description: isMobile ? "Tap the color palette to paint" : "Use Arrow Keys to change color", icon: Palette, color: "text-violet-500", bg: "bg-violet-500", border: 'border-violet-500' },
    { id: 9, title: "Pause / Menu", description: isMobile ? "Tap the top-right menu button" : "Press 'ESC' to pause the game", icon: Menu, color: "text-slate-500", bg: "bg-slate-500", border: 'border-slate-500' },
      ...
      {step === 0.5 ? (
        ...
        >
          <div className="bg-white rounded-[2rem] shadow-2xl max-w-[360px] w-full p-8 text-center flex flex-col items-center relative overflow-hidden border border-white/50">
            ...
            <div className={`w-24 h-24 rounded-[2rem] flex items-center justify-center mb-6 bg-slate-50 shadow-inner border border-slate-100 ${currentStep.color} relative overflow-hidden group`}>
               <div className={`absolute inset-0 ${currentStep.bg} opacity-10`} />
               <motion.div
            ...
            >
              ...
              <span className="relative z-10">{step === 0 ? "Start Tutorial" : "Finish Tutorial"}</span>
        ...
        >
          <div className="bg-white/95 backdrop-blur-xl border border-white/60 shadow-2xl rounded-2xl p-4 flex items-center gap-4 max-w-sm w-full relative overflow-hidden"
               style={{ color: 'inherit' }}>
            ...
            <div className={`shrink-0 w-12 h-12 rounded-[1rem] flex items-center justify-center ${currentStep.bg} text-white shadow-inner relative z-10`}>
              <Icon className="w-6 h-6" strokeWidth={2.5} />
            </div>
            
            <div className="flex-1 font-sans relative z-10 pt-0.5">
              <div className={`text-[10px] font-black tracking-widest uppercase ${currentStep.color} mb-0.5`}>
                Step {step} of 9
              </div>
            ...
            >
              <div className={`w-3 h-3 rounded-full ${currentStep.bg} shadow-inherit`} />
            </motion.div>
          </div>
        </motion.div>
      )}
